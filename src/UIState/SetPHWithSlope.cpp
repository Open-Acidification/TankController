...
float SetPHSetPoint::getCurrentValue() {
  if (subState == 0) {
    ...
    if (PHControl::instance()->getPHSetType() == 2) {
      return PHControl::instance()->getRampStartingPH();
    } else {
      return 0.000;
    }
  } else {
    if (PHControl::instance()->getPHSetType() == 2) {
      ...
      return PHControl::instance()->getRampTimeStart() / 3600;
    }
  }
}
...
void SetPHSetPoint::setValue(float value) {
  ...
  if (subState < NUM_VALUES) {
    ...
    PHControl::instance()->setSlope(values[1], values[2]);
    char output0[17];
    char output1[17];
    char output2[17];
    snprintf_P(output0, sizeof(output0), (PGM_P)F("New pH=%i.%i"), (int)values[0],
               (int)(values[0] * 1000 + 0.5) % 1000);
    snprintf_P(output1, sizeof(output1), (PGM_P)F("A=%i.%i"), (int)value, (int)(value * 1000) % 1000);
    snprintf_P(output1, sizeof(output1), (PGM_P)F("P=%i.%i"), (int)value, (int)(value * 1000) % 1000);
