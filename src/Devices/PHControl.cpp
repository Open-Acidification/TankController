...
void PHControl::setTargetPh(double newPh) {
  ...
  serialWithTime((const char *)F("set target pH to %6.4f"), newPh);
}

void PHControl::setUsePID(bool flag) {
  usePID = flag;
  // save to EEPROM?
  serialWithTime((const char *)(flag ? F("enable PID") : F("disable PID")));
}
...
void PHControl::updateControl(double pH) {
  ...
  if (newValue != oldValue) {
    serialWithTime((const char *)(newValue ? F("CO2 bubbler off") : F("CO2 bubbler on")));
